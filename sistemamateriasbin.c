/*
	...
	El manejo de la informacion se realiza en un archivo binario (bin).
*/
...
void Menu();
void CapturaDatos(ALUMNO aAl[n], int iCont);
void CapturaMaterias(ALUMNO aAl[n], int iCont);
void AgregaArchivo(char cArchivoF[15], ALUMNO aAl[n], int iCont);
...
void Menu(){
	...
	do{
		...
		system("cls");
		switch(cOp){
			case '1': CapturaDatos(aAlumnos, iContador);
			          CapturaMaterias(aAlumnos, iContador);
			          AgregaArchivo("Bases2021.bin", aAlumnos, iContador);
					  iContador++;
					  break;
			case '2': BajaAlumno(aAlumnos, iContador);
					  Sobreescribe("Bases2021.bin", aAlumnos, iContador);
					  break;
...
void AgregaArchivo(char cArchivoF[15], ALUMNO aAl[n], int iCont){
	FILE *fArchivo;
	fArchivo=fopen(cArchivoF,"ab");
	fwrite(&aAl[iCont],sizeof(ALUMNO),1,fArchivo);
	fclose(fArchivo);
}
